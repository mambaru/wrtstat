...
public:
  ...
  /*
  typedef typename manager_type::simple_adder_t simple_adder_t;
  ...
  */
  ...
  explicit meter_manager(const options_type& opt = options_type(), id_t init = 0, id_t step = 1 ) 
    :  aggregator_registry(opt, opt.pool_size, init, step)
...
#error Допилить все остальное на фабрики 
// composite_meter
  template<typename D >
  ...
  */
    ...
    create_composite_meter_(id_t time_id, id_t read_id, id_t write_id, time_type ts_now, size_type size)
  {
    return std::make_shared<composite_meter<D> >( 
      time_id!=static_cast<id_t>(-1) ? this->create_time_meter< D >(time_id, ts_now, 1) : nullptr,
      read_id!=static_cast<id_t>(-1) ? this->create_size_meter(read_id, ts_now, size) : nullptr,
      write_id!=static_cast<id_t>(-1) ? this->create_size_meter(write_id, ts_now, 0 ) : nullptr
    );
  }
